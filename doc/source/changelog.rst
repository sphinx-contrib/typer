...
* Implemented `Support automatic light/dark theme switching based on Sphinx theme <https://github.com/sphinx-contrib/typer/issues/62>`_
  ...
  ``sphinx_book_theme``.
* Fixed: a missing ``cairosvg`` package is now reported as an error when converting svg to pdf
  instead of silently producing no pdf.
...
* Dropped support for Sphinx 6. Sphinx 7.0+ is now required.
